...
const BookingDetailPage = ({
  ...
  const bookingHours = Number(bookingTime.split(":")[0])
  // popup
      ...
      <UserInfoPageLayout childWidth="100%" paddingTop="">
        ...
        {!loading && (
          <div className="fae--booking-detail-page-main-container">
           ...
           { console.log(bookingDurationTime(bookingDate), bookingDate, 'bookingDate')}
            {bookingDurationTime(bookingDate) >= 24 && 
              <>
                {completeSession > 0 ? (
                  ""
                ) : (  
                  ...
                  <FAEText
                    onClick={() => 
                      history.push({
                        ...
                        state: { ...bookingDetails,serviceId:serviceTypeId, providerId:providerId, bookingId:state?.bookingId, businessId:state?.businessId, cartId: state.cartId, sessionId:sessionId},
                      }) 
                    }
                    className="fae--bokking-detail-action-button">
                    <img
